...
from halotools.sim_manager import manipulate_ptcl_table_cache_log
...
fname_cache_log = manipulate_ptcl_table_cache_log.get_ptcl_table_cache_log_fname()
...
corrupted_cache_log_fname = 'corrupted_ptcl_table_cache_log.txt'
...
def fnames_in_existing_log():
    ...
    """
    try:
        manipulate_ptcl_table_cache_log.verify_ptcl_table_cache_log()
        existing_log = manipulate_ptcl_table_cache_log.read_ptcl_table_cache_log()
...
def ptcl_table_fnames_in_standard_cache():
    """
    """
    standard_loc = os.path.join(os.path.dirname(fname_cache_log), 'particle_catalogs')
...
print("\nNumber of files detected in standard cache location = " 
    + str(len(list(ptcl_table_fnames_in_standard_cache()))) + "\n")
...
potential_fnames.extend(list(ptcl_table_fnames_in_standard_cache()))
...
def verified_fname_generator():
    ...
    for fname in potential_fnames:
        try:
            verified_fname = manipulate_ptcl_table_cache_log.verify_file_storing_unrecognized_ptcl_table(fname)
...
if len(new_log) > 0:
    manipulate_ptcl_table_cache_log.overwrite_ptcl_table_cache_log(new_log, cache_fname=fname_cache_log)
...
if len(rejected_fnames) > 0:
    ...
    "you can find out why the file was rejected by reading the traceback triggered by the following syntax:\n\n"
        ">>> from halotools.sim_manager import manipulate_ptcl_table_cache_log \n"
        ">>> manipulate_ptcl_table_cache_log.verify_file_storing_unrecognized_ptcl_table(fname) \n")
