...
class RockstarReader(object):
    ...
    def read_halocat(self, fname, input_column_info, input_halo_cuts=None,
        ...
        input_halo_cuts : array_like, optional 
            List whose entries are tuples providing information 
        ...
        # If applying cuts, make sure input_halo_cuts contains a subset 
        # of columns in input_column_info. Append to input_column_info if not. 
        if input_halo_cuts is not None:
            halo_cuts = reformat_cuts(input_halo_cuts) # Switches None cuts entries to float("inf")
            column_info = add_missing_columns(input_column_info, halo_cuts)
        else:
            halo_cuts=None
            ...
            for i in range(Ndata_rows):
                ...
                if parsed_line != []:
                    ...
                    if halo_cuts == None:
                        ...
                        keep_row = halocut(parsed_line, column_info, halo_cuts)
                        if keep_row == 1:
                            output.append(line)

        # Now bundle up the result into a structured numpy array
        def get_dt_string(self, type_list):
    ...
    def halocut(self, data_row, column_info, halocuts):
        """ Determine whether the input halo should be kept or discarded 
        based on a series of input cuts. 

        Parameters 
        ----------
        data_row : array_like 
            array of strings corresponding to a single row of the halo table. 

        column_info : array_like 
            List of tuples provided as input to `read_halocat` to determine 
            which columns of the halo catalog are saved. 
            See docstring of `read_halocat` for details. 

        halocuts : array_like 
            List of tuples provided as input to `read_halocat` to determine 
            how cuts are applied to the halo catalog. 
            See docstring of `read_halocat` for details. 

        Returns 
        -------
        output : boolean 
            True if the row contains data for a halo that passes the cuts, 
            False if not. 
        """
        output = True
        for cut in halocuts:
    ...
    def add_missing_columns(self, input_column_info, cuts):
        """ Augments input_column_info with columns used to 
        apply cuts to the halo data, if applicable. 
    ...
    def reformat_cuts(self, input_cuts):
