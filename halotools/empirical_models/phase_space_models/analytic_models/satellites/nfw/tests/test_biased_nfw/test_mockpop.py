...
def _radial_velocities(galaxy_sample, Lbox):
    ...
    xrel, vxrel = relative_positions_and_velocities(
        galx, hostx, v1=galvx, v2=hostvx, period=Lbox[0]
    )
    ...
    yrel, vyrel = relative_positions_and_velocities(
        galy, hosty, v1=galvy, v2=hostvy, period=Lbox[1]
    )
    ...
    zrel, vzrel = relative_positions_and_velocities(
        galz, hostz, v1=galvz, v2=hostvz, period=Lbox[2]
    )
    r = np.sqrt(xrel**2 + yrel**2 + zrel**2)
    vr = np.zeros_like(r)
    mask = r > 0
    vr[mask] = (
        xrel[mask] * vxrel[mask] + yrel[mask] * vyrel[mask] + zrel[mask] * vzrel[mask]
    ) / r[mask]
    return vr
...
def enforce_correct_conc_gal_bias(galaxy_sample, correct_conc_gal_bias_value):
    ...
    satmsg = (
        "``galaxy_sample`` has satellites with `conc_gal_bias` that disagrees with {0:.3f}"
        "".format(correct_conc_gal_bias_value)
    )
    assert np.all(
        galaxy_sample["conc_gal_bias"][satmask] == correct_conc_gal_bias_value
    ), satmsg
    cenmsg = "``galaxy_sample`` has centrals with non-zero `conc_gal_bias`"
    assert np.all(galaxy_sample["conc_gal_bias"][~satmask] == 0.0), cenmsg
...
def mean_satellite_host_centric_dimensionless_distance(galaxy_sample):
    satmask = galaxy_sample["gal_type"] == "satellites"
    r_by_rvir = (
        galaxy_sample["host_centric_distance"][satmask]
        / galaxy_sample["halo_rvir"][satmask]
    )
...
def test_mockpop1():
    ...
    r_by_rvir_0p1 = mean_satellite_host_centric_dimensionless_distance(
    ...
    mask16_0p1 = _nearest_lower_value_mask(
        sats_0p1["halo_mvir"], min_host_mass_to_search_for
    )
    ...
    r_by_rvir_10 = mean_satellite_host_centric_dimensionless_distance(
        model.mock.galaxy_table
    )
    ...
    min_host_mass_to_search_for = 10**16.001
    mask16_10 = _nearest_lower_value_mask(
        sats_10["halo_mvir"], min_host_mass_to_search_for
    )
...
def test_mockpop2():
    ...
    mask14 = _nearest_lower_value_mask(sats["halo_mvir"], min_host_mass_to_search_for)
    ...
    min_host_mass_to_search_for = 10**16.001
    mask16 = _nearest_lower_value_mask(sats["halo_mvir"], min_host_mass_to_search_for)
    ...
    r_by_rvir_m14 = mean_satellite_host_centric_dimensionless_distance(
        galaxy_sample_m14
    )
    r_by_rvir_m16 = mean_satellite_host_centric_dimensionless_distance(
        galaxy_sample_m16
    )
    msg = "mean host_centric_distance should decrease when increasing ``conc_gal_bias`` param"
    ...
    inner_mask = (
        galaxy_sample_m16["host_centric_distance"] / galaxy_sample_m16["halo_rvir"]
        < 0.5
    )

    msg = (
        "Galaxies in inner-half of the halo should have larger radial velocity dispersions \n"
