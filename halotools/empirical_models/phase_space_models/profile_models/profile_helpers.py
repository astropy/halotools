...
def density_threshold(cosmology, redshift, mdef):
    ...
    Notes
    ------
    This function is tested by the `~halotools.empirical_models.test_profile_helpers.TestProfileHelpers.test_density_threshold` function
    and the `~halotools.empirical_models.test_profile_helpers.TestProfileHelpers.test_density_threshold_error_handling` function.
    """
...
def delta_vir(cosmology, redshift):
    ...
    Notes
    ------
    This function is tested by the `~halotools.empirical_models.test_profile_helpers.TestProfileHelpers.test_delta_vir` function.
    """
    try:
        assert isinstance(cosmology, astropy_cosmology_obj.core.FLRW)
    except AssertionError:
...
def halo_mass_to_halo_radius(mass, cosmology, redshift, mdef):
    ...
    Notes
    ------
    This function is tested by the `~halotools.empirical_models.test_profile_helpers.TestProfileHelpers.test_halo_radius_to_halo_mass` function.
    """
    try:
        assert isinstance(cosmology, astropy_cosmology_obj.core.FLRW)
    except AssertionError:
