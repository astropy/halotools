...
def test_Num_ptcl_requirement():
    ...
    del model.mock
    model.populate_mock(halocat, Num_ptcl_requirement=0.0)
    assert model.mock.Num_ptcl_requirement == 0.0
...
def test_factory_constructor_redshift3():
    ...
    model2 = HodModelFactory(baseline_model_instance=model1, redshift=model1.redshift)

    assert model2.redshift == model1.redshift

    with pytest.raises(AssertionError) as err:
        model3 = HodModelFactory(
            baseline_model_instance=model1, redshift=model1.redshift + 1
        )
    substr = "that is inconsistent with the redshift z ="
    assert substr in err.value.args[0]
...
def test_raises_appropriate_exception1():
    ...
    with pytest.raises(HalotoolsError) as err:
        model.build_model_feature_calling_sequence(
            {"model_feature_calling_sequence": (4, 5)}
        )
    substr = (
        "does not appear in the keyword arguments you passed to the HodModelFactory"
    )
    assert substr in err.value.args[0]
...
def test_raises_appropriate_exception2():
    ...
    model = HodModelFactory(**model_dict_no_redshift)
    with pytest.raises(HalotoolsError) as err:
        model._test_model_feature_calling_sequence_consistency(["abc"], model.gal_types)
