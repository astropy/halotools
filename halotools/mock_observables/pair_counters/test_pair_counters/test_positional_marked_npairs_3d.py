...
__all__ = (
    ...
    "test_compare_to_pure_python_result",
)
...
def compute_limiting(coords1, coords2, npts, rbins, weight_func_id, alignment):
    # generate vectors parallel, perpendicular or antiparallel to separation vector
    # (note that this should be two point clouds
    # rather than a range of particle positions)
    diff_coords = coords2 - coords1
    norm_value = np.sqrt(
        diff_coords[:, 0] ** 2 + diff_coords[:, 1] ** 2 + diff_coords[:, 2] ** 2
    )
...
def test_1():
    ...
    assert np.isclose(
        weighted_counts[-1], avg_costheta * counts[-1], rtol=1.0 / npts
    ), msg
...
def test_2():
    ...
    msg = "weighted counts do not match expected result given the weighting function"
    assert np.isclose(
        weighted_counts[-1], avg_two_costheta * counts[-1], rtol=1.0 / npts
    ), msg
...
def test_3():
    ...
    msg = "weighted counts do not match expected result given the weighting function"
    assert np.isclose(
        weighted_counts[-1], avg_two_sintheta * counts[-1], rtol=1.0 / npts
    ), msg
...
def test_randoms():
    ...
    # generate orientation vectors for cluster 1
    vectors1 = normalized_vectors(np.random.random((npts, 3)) * 2.0 - 1.0)
    ...
    msg = (
        "weighted counts do not match expected result given the weighting function"
        + " "
        + str(np.max(dist_a))
    )
...
def test_threading():
    """
    test to make sure the result is the same
    with and without threading for each weighting function
    """
    ...
    weighted_counts_1, counts_1 = positional_marked_npairs_3d(
        random_coords,
        random_coords,
        rbins,
        period=period,
    ...
    weighted_counts_2, counts_2 = positional_marked_npairs_3d(
        random_coords,
        random_coords,
        rbins,
        period=period,
...
def test_unweighted_counts():
    ...
    counts_2 = npairs_3d(
        random_coords, random_coords, rbins, period=period, num_threads=3
    )
...
def test_compare_to_pure_python_result():
    ...
    weighted_counts_2, counts_2 = cos2theta_pairs(
        random_coords, random_vectors, random_coords, rbins, period=period
    )
