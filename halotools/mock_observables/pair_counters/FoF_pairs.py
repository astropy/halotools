...
def fof_pairs(data1, data2, r_max, Lbox=None, period=None, verbose=False, N_threads=1):
    ...
    if N_threads>1:
        d, i_inds, j_inds = pool.map(engine,range(Ncell1))
    if N_threads==1:
        d, i_inds, j_inds = map(engine,range(Ncell1))
...
def _fof_pairs_engine(grid1, grid2, rbins, period, PBCs, icell1):
    """
    ...
    for icell2 in adj_cell_arr:
        ...
        if PBCs==False:
            dd, ii_inds+i_min, jj_inds+j_min = pairwise_distance_no_pbc_no_pbc(x_icell1, y_icell1, z_icell1,\
                                                      x_icell2, y_icell2, z_icell2,\
                                                      r_max)
        else: #PBCs==True
            dd, ii_inds+i_min, jj_inds+j_min = pairwise_distance_no_pbc_pbc(x_icell1, y_icell1, z_icell1,\
                                                   x_icell2, y_icell2, z_icell2,\
                                                   period, r_max)
