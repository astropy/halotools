...
void countpairs_pbc(const int ND1, const double * const X1, const double * const Y1, const double  * const Z1,
				...
				const int nrpbin, const double * restrict rupp, int **paircounts)
{
  ...
  cellarray *lattice1 = gridlink_pbc(ND1, X1, Y1, Z1, xmin, xmax, ymin, ymax, zmin, zmax, rpmax, bin_refine_factor, bin_refine_factor, bin_refine_factor, &nmesh_x, &nmesh_y, &nmesh_z);
  cellarray *lattice2 = NULL;
  if(autocorr==0) {
	int ngrid2_x=0,ngrid2_y=0,ngrid2_z=0;
	lattice2 = gridlink_pbc(ND2, X2, Y2, Z2, xmin, xmax, ymin, ymax, zmin, zmax, rpmax, bin_refine_factor, bin_refine_factor, bin_refine_factor, &ngrid2_x, &ngrid2_y, &ngrid2_z);
