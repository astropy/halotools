...
from ..reduced_inertia_tensor import reduced_inertia_tensor_per_object, rotation3d
...
def test_reduced_inertia_tensor3():
    ...
    ed_max = 1.0
    tensors_1, sum_of_masses1_1 = reduced_inertia_tensor_per_object(sample1, sample2, rsmooth, period=Lbox, inertia_tensor_0=tensor_0, ed_max=ed_max)
...
def test_reduced_inertia_tensor5():
    """
    """
    ...
    sample1 = np.array([[0.5,0.5,0.5]])
    # sample an ellipsoidal surface with the major (minor) axis aligned the the z-axis (x-axis)
    ran_axis = np.random.random(3)
    sample2 = sample_ellipsoidal_surface(N2, q0, s0, ran_axis, np.pi/2.0)*0.05 + 0.5

    # give equal weight to each point
    masses2 = np.ones(N2)

    # get basis vectors for eigen frame
    rot_m = rotation_matrices_from_angles(np.pi/2.0, ran_axis)
    us = np.array([[1,0,0],[0,1,0],[0,0,1]])
    u1, u2, u3 = rotate_vector_collection(rot_m, us)
    a , b, c = 1.0, q0**2, s0**2

    # construct inertia tensor
    M = np.matrix(np.array([[1, 0, 0],[0,b,0],[0,0,c]]))
    S = np.matrix(np.array([u1,u2,u3]).T)
    tensor_0 = np.array([S*M*S.getI()])
