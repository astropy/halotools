...
__all__ = ('mean_ds_12h_rhalo_engine', )

@cython.boundscheck(False)
@cython.wraparound(False)
@cython.nonecheck(False)
def mean_ds_12h_rhalo_engine(double_mesh, zperiod_in, x1in, y1in, z1in, rhalo1in, x2in, y2in, z2in, m2in, rp_bins, cell1_tuple):
    ...
    cdef cnp.float64_t[:] rhalo1_sorted = np.ascontiguousarray(
        rhalo1in[double_mesh.mesh1.idx_sorted], dtype=np.float64)
    ...
    cdef cnp.float64_t x2shift, y2shift, z2shift, dx, dy, dz, dxy_sq, dsq, dzsq
    cdef cnp.float64_t x1tmp, y1tmp, z1tmp, m2tmp
    ...
    cdef cnp.float64_t[:] rhalo_icell1
    cdef cnp.float64_t r1tmp, rhalosq
    ...
    for icell1 in range(first_cell1_element, last_cell1_element):
        ...
        rhalo_icell1 = rhalo1_sorted[ifirst1:ilast1]
                    ...
                    if Nj > 0:
                        for i in range(0, Ni):
                            ...
                            for j in range(0, Nj):
                                ...
                                dxy_sq = dx*dx + dy*dy
                                dzsq = dz*dz
                                if dzsq > zperiod*zperiod/4.:
                                    dzsq -= zperiod*zperiod/4.
                                dsq = dxy_sq + dzsq
