...
__all__ = ('weighted_npairs_per_object_xy_engine', )

@cython.boundscheck(False)
@cython.wraparound(False)
@cython.nonecheck(False)
def weighted_npairs_per_object_xy_engine(double_mesh, x1in, y1in, x2in, y2in, w2in, rp_bins, cell1_tuple):
    ...
    cdef cnp.float64_t[:] w2_sorted = np.ascontiguousarray(
        w2in[double_mesh.mesh2.idx_sorted], dtype=np.float64)

    cdef cnp.float64_t[:] weighted_inner_counts = np.zeros((num_rp_bins,), dtype=np.float64)
    cdef cnp.float64_t[:, :] weighted_outer_counts = np.zeros(
        (len(x1_sorted), num_rp_bins), dtype=np.float64)
                        ...
                        for i in range(0, Ni):
                            ...
                            for j in range(0, Nj):
                                ...
                                while dxy_sq <= rp_bins_squared[k]:
                                    weighted_inner_counts[k] += w2tmp
                                    k=k-1
                                    if k<0: break

                            # update the outer counts
                            for k in range(0, num_rp_bins):
                                weighted_outer_counts[ifirst1 + i, k] += weighted_inner_counts[k]
                                weighted_inner_counts[k] = 0 #re-zero the inner counts
    ...
    sorted_weighted_counts = np.array(weighted_outer_counts)
    idx_unsorted = unsorting_indices(double_mesh.mesh1.idx_sorted)
    return sorted_weighted_counts[idx_unsorted, :]
