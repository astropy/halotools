...
def test_delta_sigma_consistency6():
    """Enforce consistent results between halo_radii and halo_id inputs
    """
    galaxies = generate_centers(4, 1)  # {0.1, 0.3, 0.5, 0.7, 0.9}
    ds, ns = 0.2, 5
    ixg = np.array(galaxies[:, 0] // ds).astype(int)
    iyg = np.array(galaxies[:, 1] // ds).astype(int)
    izg = np.array(galaxies[:, 2] // ds).astype(int)
    galaxy_halo_ids = np.ravel_multi_index((ixg, iyg, izg), (ns, ns, ns))
    halo_radii = np.zeros(galaxies.shape[0]) + 0.05

    ptcl_centers_1h = generate_centers(4, 1)
    ixp = np.array(ptcl_centers_1h[:, 0] // ds).astype(int)
    iyp = np.array(ptcl_centers_1h[:, 1] // ds).astype(int)
    izp = np.array(ptcl_centers_1h[:, 2] // ds).astype(int)
    icell_1h = np.ravel_multi_index((ixp, iyp, izp), (ns, ns, ns))

    ptcl_centers_2h = generate_centers(8, 1)
    icell_2h = np.zeros(ptcl_centers_2h.shape[0]).astype('i8') + int(ns*ns*ns)

    particles = np.concatenate((ptcl_centers_1h, ptcl_centers_2h), axis=0)
    particle_halo_ids = np.concatenate((icell_1h, icell_2h))
    particle_masses = np.zeros_like(particle_halo_ids) + 1.
    ...
    ds_1h_per_obj, ds_2h_per_obj = mean_delta_sigma_one_two_halo_decomp(
        galaxies, particles, particle_masses,
        downsampling_factor, rp_bins, period=Lbox,
        galaxy_halo_ids=galaxy_halo_ids, particle_halo_ids=particle_halo_ids)

    ds_1h_per_obj2, ds_2h_per_obj2 = mean_delta_sigma_one_two_halo_decomp(
        galaxies, particles, particle_masses,
        downsampling_factor, rp_bins, period=Lbox,
        halo_radii=halo_radii)

    assert np.allclose(ds_1h_per_obj, ds_1h_per_obj2)
    assert np.allclose(ds_2h_per_obj, ds_2h_per_obj2)
...
def generate_centers(npts, Lbox):
    x = np.linspace(0, Lbox, npts+1)
    y = np.linspace(0, Lbox, npts+1)
    z = np.linspace(0, Lbox, npts+1)
    delta = np.diff(x)[0]/2.
    x, y, z = np.array(np.meshgrid(x[:-1], y[:-1], z[:-1]))
    return np.vstack([x.flatten()+delta, y.flatten()+delta, z.flatten()+delta]).T


def gen_cluster(npts, xc, yc, zc, epsilon=0.01):
