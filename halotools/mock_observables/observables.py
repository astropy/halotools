...
def two_point_correlation_function_jackknife(sample1, randoms, rbins, Nsub=10, Lbox=[250.0,250.0,250.0], sample2 = None, 
    ...
    N_sub_vol = np.prod(Nsub) # the number of subvolumes
    
    #tag each particle with an integer indicating which jackknife subvolume it falls within
    #subvolume indices for the sample1 particle's positions
    index1 = np.sum(np.floor(sample1/dL)*np.hstack((1,np.cumprod(Nsub[:-1]))),axis=1)+1
    ...
    index2 = np.sum(np.floor(sample2/dL)*np.hstack((1,np.cumprod(Nsub[:-1]))),axis=1)+1
    index2 = index2.astype(int)
    
    
    def jnpair_counts(sample1, sample2, index1, index2, N_sub_vol, rbins, period, N_thread):
        """
        Count jackknife pairs.
        """
        if N_threads==1:
            D1D1 = jnpairs(sample1, sample1, rbins, period=period,  weights1=index1, weights2=index1, N_vol_elements=N_sub_vol)
            D1D1 = np.diff(D1D1,axis=1)
            if np.all(sample1 != sample2):
                D1D2 = jnpairs(sample1, sample2, rbins, period=period, weights1=index1, weights2=index2, N_vol_elements=N_sub_vol)
                D1D2 = np.diff(D1D2,axis=1)
                D2D2 = jnpairs(sample2, sample2, rbins, period=period, weights1=index2, weights2=index2, N_vol_elements=N_sub_vol)
            ...
            args = [[sample1[chunk],sample1,rbins,period,index1[chunk],index1,N_sub_vol] for chunk in np.array_split(inds1,N_threads)]
            D1D1 = np.sum(pool.map(_jnpairs_wrapper,args),axis=0)
            D1D1 = np.diff(D1D1,axis=1)
            if np.all(sample1 != sample2):
                args = [[sample1[chunk],sample2,rbins,period,index1[chunk],index2,N_sub_vol] for chunk in np.array_split(inds1,N_threads)]
                D1D2 = np.sum(pool.map(_jnpairs_wrapper,args),axis=0)
                D1D2 = np.diff(D1D2,axis=1)
                args = [[sample2[chunk],sample2,rbins,period,index2[chunk],index2,N_sub_vol] for chunk in np.array_split(inds2,N_threads)]
    ...
    if np.all(sample1==sample2):
        D1D1, D1R, RR = jnpair_counts(sample1, randoms, index1, index_random, N_sub_vol, rbins, period, N_threads)
        #the first row of the results are the full sample pair counts
        D1D1_full = D1D1[0,:]
        D1R_full = D1R[0,:]
        RR_full = RR[0,:]
        #the remaining rows are for each of the subsamples
        D1D1_sub = D1D1[1,:]
        D1R_sub = D1R[1,:]
        RR_sub = RR[1,:]
    else:
        D1D1, D1D2, D2D2 = jnpair_counts(sample1, sample2, index1, index_random, N_sub_vol, rbins, period, N_threads)
        D1D2_full = D1D2[0,:]
        D1D2_sub = D1D2[1,:]
        D1D1, D1R, RR = jnpair_counts(sample1, randoms, index1, index_random, N_sub_vol, rbins, period, N_threads)
        D1D1_full = D1D1[0,:]
        ...
        D1R_sub = D1R[1,:]
        RR_sub = RR[1,:]
        D2D2, D2R, RR = jnpair_counts(sample2, randoms, index1, index_random, N_sub_vol, rbins, period, N_threads)
        D2D2_full = D1D1[0,:]
        D2R_full = D1DR[0,:]
        RR_full = RR[0,:]
        D2D2_sub = D1D1[1,:]
    ...
    if np.all(sample2==sample1):
        xi_11 = TP_estimator(D1D1_full,D1R_full,RR_full,factor1,estimator)
        return xi_11
    else:
        xi_11 = TP_estimator(D1D1_full,D1R_full,RR_full,factor1,estimator)
        xi_12 = TP_estimator(D1D2_full,D1R_full,RR_full,factor1,estimator)
        xi_22 = TP_estimator(D2D2_full,D2R_full,RR_full,factor2,estimator)
        return xi_11, xi_12, xi_22
